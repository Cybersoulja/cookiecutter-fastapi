...
1. **FastAPI service** (`app/`)
   - `main.py` builds the `FastAPI` app, mounts `api_router` under `API_PREFIX` (`/api`), and *optionally* wires an app-startup handler (`pre_load` is hardcoded `False`, so `create_start_app_handler`/model preloading is currently dead code unless enabled).
...
- Deployment targets: Docker/`docker-compose.yml` (local), GCP Cloud Run (`Dockerfile`), and AWS Lambda (`mangum`, an optional poetry dependency group `aws`) — see the generated `README.md` for the corresponding `gcloud`/`sam` commands.
